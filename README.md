...
3. 使わない音声は `Mute` にします。
4. `Model` で Whisper モデルを選びます。`faster-whisper` が提供する全モデルを選択できます。
...
- `large-v3` / `large-v3-turbo` / `turbo`: 精度や速度を重視する場合の候補。より多くの計算資源が必要
- `.en` または `distil-*.en`: 英語用モデルのため、日本語会議には適しません
